...
<div id="htmlcontent_{$hook|escape:'htmlall':'UTF-8'}"{if $hook == 'footer'} class="footer-block col-xs-12 col-sm-4"{/if}>
	<ul class="htmlcontent-home clearfix row">
		{foreach name=items from=$htmlitems item=hItem}
			...
			{/if}
					{if $hItem.url}
						<a onClick="ga('ec:addPromo', { 'id': 'PROMO_BANER', 'name': 'Baner boczny', 'position': 'panel_boczny'});" onClick="ga('ec:setAction', 'promo_click');" onClick="ga('send', 'event', 'Promocje wewnętrzne', 'Baner boczny', 'Kategoria ' + '{$hItem.title|escape:'htmlall':'UTF-8'}');" href="{$hItem.url|escape:'htmlall':'UTF-8'}" class="item-link"{if $hItem.target == 1} onclick="return !window.open(this.href);"{/if} title="{$hItem.title|escape:'htmlall':'UTF-8'}">
					{/if}
						{if $hItem.image}
